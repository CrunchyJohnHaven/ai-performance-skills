...
- **Local-only operation.** Every tick, query, and closet rebuild runs on the employee's machine. No data traverses the network unless the employee explicitly triggers the managed flow or the remote health check. There is no always-on outbound connection, no background sync, and no cloud dependency for normal use.
