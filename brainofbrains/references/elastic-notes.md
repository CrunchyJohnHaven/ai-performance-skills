...
- **Privacy is the default, not a policy promise.** Every tick, query, and closet rebuild runs entirely on your own machine. After bootstrap, nothing leaves unless you explicitly run `scripts/provision.sh` or `scripts/health.sh --remote`. There is no phone-home, no background telemetry, and no local server listening for connections.
