...
2. **Run a shadow-mode A/B on code-review calls** — `code-review` is your top cost category at $0.57 over 4 calls this week. Run `npx kostai shadow --task code-review` to compare claude-sonnet against claude-haiku on the same inputs. If quality holds (elasticjudge score delta < 0.5 across axes), switch the default for that task type permanently.

3. **Wire the prose compressor to your memory files** — if your workspace uses a `CLAUDE.md` or memory directory longer than 2,000 tokens, the compressor can drop 40–50% of input tokens on every call that reads it. Run `npx kostai scan` to identify the exact file paths being loaded.
...
_Generated by `scripts/proof.sh --audience demo --date 2026-04-22` | ai-cost v0.4.0_
