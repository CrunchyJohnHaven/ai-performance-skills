...
Coordinators: build the email attachment with `npm run package:elastic-pilot-zip` from the KostAI repo root (writes `dist/elastic-ai-performance-skill-pilot.zip`). Outbound blurb: `assets/elastic-pilot-coordinator-wrapper.txt`. Package includes **`START_HERE.txt`** at zip root (and under `skills/cost-optimization/assets/` on GitHub). **Default first run:** unzip or GitHub ZIP → read **START_HERE.txt** → **PILOT_README.txt** → paste **ELASTIC_PILOT_PROMPT.txt** (zip) or **assets/elastic-pilot-participant-prompt.txt** (clone) into Claude; assistant runs **`pilot-complete.sh`**. Day ~30: `PILOT_LEDGER_ROOT=… pilot-30d-report.sh` → `deliverables/elastic-pilot-30d-<date>/FEEDBACK.md`.

Five steps for an employee participating in the internal pilot.

1. **Install the skill.** Unzip the pilot package or clone the repo or run `npm install -g @sapperjohn/kostai` and symlink `skills/cost-optimization/` into `~/.claude/skills/cost-optimization/`, or install via the Agent Builder catalog if it is already published there.
