...
Do not edit source code outside of ai-cost.config.json, .ai-cost-data/, and .kostai/ unless I
...
When publishing to the internal Agent Builder skills catalog from this repo, the folder is self-contained — ship `cost-optimization/` as-is. If publishing from the npm package instead, use `skills/cost-optimization/`. No rename required; the frontmatter `name: cost-optimization` is the canonical identifier.
